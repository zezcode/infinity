...
<html lang="vi">
	<head>
		<title>{$config['website_name']} - reset passwd</title>
		<meta charset="utf-8" />
	...
	<style>body { background-image: url({if (is_null($config['website_auth_background_image']))}'/theme/infinity/media/auth/bg4.jpg'{else}{$config['website_auth_background_image']}{/if}); } [data-bs-theme="dark"] body { background-image: url({if (is_null($config['website_auth_background_image']))}'/theme/infinity/media/auth/bg4-dark.jpg'{else}{$config['website_auth_background_image']}{/if}); }</style>
						...
						<div class="card-body p-10 p-lg-20">
							<form class="form w-100" novalidate="novalidate" id="kt_password_reset_form" action="#">
								<div class="text-center mb-11">
										<h1 class="text-dark fw-bolder mb-3">reset passwd</h1>
