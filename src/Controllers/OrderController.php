            ...
            case 3: //续费产品
                try {
                    ...
                    if (is_null($latest_order->product_period)) {
                        throw new \Exception('Lỗi đơn hàng, không thể gia hạn');
                    }
                    $product = Product::find($user->product_id);
                    if (is_null($product)) {
                        throw new \Exception('Gói đã bị xóa và không thể gia hạn');
                    }
                    if ($product->renew === 0 && $product->status === 0) {
                        throw new \Exception('Gói này không cho phép gia hạn');
                    }
