...
namespace App\Payments\Momo;

final class MomoSubmit
{
    private $config;
    ...
    public function buildRequestMysign($para_sort)
    {
        ...
        $prestr = MomoTool::createLinkstring($para_sort);

        return MomoTool::md5Sign($prestr, $this->config['key']);
    }

    public function buildRequestPara($para_temp)
    {
        //除去待签名参数数组中的空值和签名参数
        $para_filter = MomoTool::paraFilter($para_temp);

        //对待签名参数数组排序
        $para_sort = MomoTool::argSort($para_filter);
    ...
    public function buildRequestParaToString($para_temp)
    {
        ...
        return MomoTool::createLinkstringUrlencode($para);
    }
    ...
    public function buildRequestPost($para_temp)
    {
        ...
        $result = MomoTool::getHttpResponsePOST($url, $para);
