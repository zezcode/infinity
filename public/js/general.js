...
function countdown(date, dom) {
  ...
  function updateTimer() {
    ...
    if (distance <= 0) {
      ...
      const countdown = `${days}${i18next.t(
        "day"
...
function kTUserConfigureProductModal(id) {
  const checkOutButton = document.querySelector(
    `[data-kt-users-action="check-out-${id}"]`
  );
    ...
    getProductData().then(function (data) {
      ...
      if (product_info.type == 1) {
        ...
        Object.entries(all_prices).forEach(([key, { label, value }]) => {
          if (value) {
            $("#zero_modal_configure_product_" + key).html(
              `<a class="btn btn-outline btn-active-light-primary" data-bs-toggle="pill">${label}</a>`
            );
            $("#zero_modal_configure_product_" + key).attr(
              "onclick",
              `KTUsersChangePlan("${value.toFixed(2)}", ${id}, "${key}")`
            );
        ...
        modalCoupon.attr(
          "onclick",
          `KTUserVerifyCoupon("${minPrice.value.toFixed(2)}", ${id})`
        );
      }

      modalInnerHtml.html(html);
      product_info.type == 3 ? modalCouponHtml.hide() : false;
      const product_final_price =
        product_info.type == 1 ? minPrice.value.toFixed(2) : onetime_price; // 判断不同类型商品的价格
      modalName.html(
        product_info.type == 1 ? name + "&nbsp;X&nbsp;" + minPrice.label : name
      );
      modalPrice.html(`${product_final_price}&nbsp;${currency_unit}`);
      modalTotal.html(`${product_final_price}&nbsp;${currency_unit}`);
      submitButton.setAttribute(
        "onclick",
        `KTUsersCreateOrder(${1}, "${product_final_price}", ${id})`
      );
...
function KTUsersChangePlan(price, id, type) {
  ...
  $("#zero_modal_configure_product_name").html(
    `${name} X ${productPlanMap[type]}`
  );
  $("#zero_modal_configure_product_price").html(
    `${price}&nbsp;${currency_unit}`
  );
  $("#zero_modal_configure_product_total").html(
    `${price}&nbsp;${currency_unit}`
  );
  submitButton.setAttribute(
    "onclick",
    `KTUsersCreateOrder(${1}, "${price}", ${id})`
  );
}
...
function KTUsersShowNodeInfo(id, userclass, nodeclass) {
  ...
  if (usersclass >= nodesclass) {
    ...
    getResult("Không có quyền làm việc này", "", "error");
  }
}
...
function oneclickImport(client, subLink) {
  quanx_config = {
    server_remote: [`${subLink}, tag=${webName}`],
  };
  var sublink = {
    surfboard: `surfboard:///install-config?url=${encodeURIComponent(subLink)}`,
    shadowrocket: `shadowrocket://add/sub://${btoa(subLink)}?remark=${encodeURIComponent(webName)}`,
    surge: `surge:///install-config?url=${encodeURIComponent(subLink)}`,
    clash: `clash://install-config?url=${encodeURIComponent(subLink)}&name=${encodeURIComponent(webName)}`,
    ...
    singbox: `sing-box://import-remote-profile?url=${encodeURIComponent(
      subLink
    )}#${encodeURIComponent(webName)}`,
  };

  Swal.fire({
    title: i18next.t("Xác nhận đồng bộ máy chủ vào App?"),
    icon: "warning",
